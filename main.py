    ...
    "🔁 `!allloop` → Herkese döngülü emoji başlatır.\n"
        ...
        if message.startswith("!katsil "):
            if not await self.is_user_allowed(user):
                await self.highrise.send_whisper(user.id, f"🚫 Bu komutu kullanamazsın @{user.username}.")
                return
            ...
            if silinecek_kat in self.kat_positions:
                ...
                self.save_kat_positions()
                await self.highrise.send_whisper(user.id, f"🗑️ '{silinecek_kat}' pozisyonu silindi.")
            else:
                await self.highrise.send_whisper(user.id, f"❓ '{silinecek_kat}' adlı pozisyon bulunamadı.")
            return

        # !katlar komutu ile tüm pozisyonları listele (Herkes kullanabilir)
        if message == "!katlar":
            if not self.kat_positions:
                await self.highrise.send_whisper(user.id, "📭 Kayıtlı hiç pozisyon yok.")
            else:
                liste = "\n".join(f"📍 {k}" for k in self.kat_positions.keys())
                await self.highrise.send_whisper(user.id, f"📦 Kayıtlı pozisyonlar:\n{liste}")
            return
        ...
        if message in self.kat_positions:
            ...
            try:
                ...
                await self.highrise.send_whisper(user.id, f"🚀 @{user.username}, '{message}' konumuna ışınlandın!")
            except Exception as e:
